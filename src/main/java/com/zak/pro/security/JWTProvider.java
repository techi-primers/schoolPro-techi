...
public class JWTProvider {
	...
	public static boolean validateJwt(String jwt, String secret) throws SignatureException, CustomException {
		Jws<Claims> parsedJwt = JWTProvider.parseJwt(jwt, secret);
		if (!JWTProvider.isExpired(parsedJwt.getBody().getExpiration())) {
			return true;
		} else {
			throw new CustomException("JWT expired");
		}
	}
	...
	public static String generateJWT(String firstName, String lastName, String email, String phoneNumber,
			boolean acceptCGU, String role, String secret, long expirationTime) {
		String jwt = Jwts.builder().setSubject(firstName + " " + lastName)
				.setExpiration(new Date(System.currentTimeMillis() + expirationTime))
				.signWith(SignatureAlgorithm.HS512, Base64.getEncoder().encodeToString(secret.getBytes()))
				.claim("role", role).claim("email", email).claim("phone", phoneNumber).claim("acceptCGU", acceptCGU)
				.compact();
		return jwt;
	}
